...
// ─── ACK Modal ────────────────────────────────────────────────────────────────
function AckModal({ dist, onClose, onDone }: { dist: Distribution; onClose: () => void; onDone: () => void }) {
  const [form, setForm] = useState<AckPayload>({
    ackMethodCode: dist.ackMethodCode, ackCode: "", ackByName: "", ackPhone: "", ackNote: "",
  });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const handleAck = async () => {
    if (!form.ackCode.trim()) { setError("Vui lòng nhập mã ACK."); return; }
    setLoading(true); setError(null);
    try { await ackDistribution(dist.id, form, getAuthSession()?.accessToken ?? ""); onDone(); }
    catch (e) { setError(e instanceof Error ? e.message : "Lỗi xác nhận"); }
    finally { setLoading(false); }
  };

  const F = (k: keyof AckPayload) => (e: React.ChangeEvent<HTMLInputElement>) => setForm(p => ({ ...p, [k]: e.target.value }));
    ...
    <div className="fixed inset-0 bg-black/40 z-[300] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="p-6 space-y-3">
          ...
          <div><label className="text-xs font-semibold text-gray-500 block mb-1">Mã ACK người dùng nhập *</label><input value={form.ackCode} onChange={F("ackCode")} placeholder="123456" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 font-mono tracking-widest text-center text-lg" /></div>
          <div><label className="text-xs font-semibold text-gray-500 block mb-1">Tên người nhận</label><input value={form.ackByName} onChange={F("ackByName")} placeholder="Trần Văn D" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" /></div>
          <div><label className="text-xs font-semibold text-gray-500 block mb-1">Số điện thoại</label><input value={form.ackPhone} onChange={F("ackPhone")} placeholder="0900000009" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" /></div>
          <div><label className="text-xs font-semibold text-gray-500 block mb-1">Ghi chú</label><input value={form.ackNote} onChange={F("ackNote")} placeholder="Đã nhận đủ..." className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" /></div>
        </div>
        <div className="flex gap-3 px-6 py-4 border-t bg-gray-50">
          ...
          <button onClick={handleAck} disabled={loading} className="flex-1 py-2.5 rounded-xl text-white font-semibold text-sm disabled:opacity-60" style={{ background: "linear-gradient(135deg,#059669,#10b981)" }}>
...
// ─── Create Modal ─────────────────────────────────────────────────────────────
function CreateModal({ onClose, onSaved }: { onClose: () => void; onSaved: (d: Distribution) => void }) {
  const [form, setForm] = useState<DistributionPayload>({
    campaignId: "", reliefPointId: "", householdId: "", incidentId: null,
    ackMethodCode: "OTP", note: "", lines: [],
  });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [newLine, setNewLine] = useState<DistributionLine>({ itemId: "", lotId: "", qty: 1, unitCode: "THUNG" });
  ...
  const handleSave = async () => {
    if (!form.householdId.trim()) { setError("Vui lòng nhập ID hộ dân."); return; }
    if (form.lines.length === 0) { setError("Cần ít nhất 1 dòng hàng."); return; }
    setLoading(true); setError(null);
    try {
      const dist = await createDistribution(form, getAuthSession()?.accessToken ?? "");
      onSaved(dist);
    } catch (e) { setError(e instanceof Error ? e.message : "Lỗi tạo phiếu"); }
    finally { setLoading(false); }
  };

  const F = (k: keyof DistributionPayload) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
    ...
    <div className="fixed inset-0 bg-black/40 z-[200] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-4">
          {error && <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-sm text-red-700">{error}</div>}
          <div>
            <label className="text-xs font-semibold text-gray-500 block mb-1">ID Hộ dân *</label>
            <input value={form.householdId} onChange={e => setForm(p => ({ ...p, householdId: e.target.value }))} placeholder="UUID hộ dân..." className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold text-gray-500 block mb-1">ID Chiến dịch</label>
              <input value={form.campaignId} onChange={e => setForm(p => ({ ...p, campaignId: e.target.value }))} placeholder="UUID..." className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 block mb-1">ID Điểm cứu trợ</label>
              <input value={form.reliefPointId} onChange={e => setForm(p => ({ ...p, reliefPointId: e.target.value }))} placeholder="UUID..." className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold text-gray-500 block mb-1">Phương thức ACK</label>
              <select value={form.ackMethodCode} onChange={e => setForm(p => ({ ...p, ackMethodCode: e.target.value }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
              <label className="text-xs font-semibold text-gray-500 block mb-1">ID Sự cố (nếu có)</label>
              <input value={form.incidentId ?? ""} onChange={e => setForm(p => ({ ...p, incidentId: e.target.value || null }))} placeholder="UUID (tùy chọn)..." className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          <div>
            <label className="text-xs font-semibold text-gray-500 block mb-1">Ghi chú</label>
            <input value={form.note} onChange={e => setForm(p => ({ ...p, note: e.target.value }))} placeholder="Phân phối theo danh sách ưu tiên..." className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          {/* Lines */}
          <div>
            ...
            <div className="flex gap-2 mb-3">
              ...
              <button onClick={addLine} className="px-3 py-2 rounded-lg bg-blue-600 text-white"><Plus size={16} /></button>
            </div>
            {form.lines.length > 0 && (
              <div className="rounded-lg border border-gray-100 overflow-hidden">
...
function statusBadge(code: string) {
  const m: Record<string, string> = {
    PENDING: "bg-amber-100 text-amber-700", ACKNOWLEDGED: "bg-emerald-100 text-emerald-700",
    CANCELLED: "bg-red-100 text-red-600",
  };
  const l: Record<string, string> = { PENDING: "Chờ nhận", ACKNOWLEDGED: "Đã nhận", CANCELLED: "Đã hủy" };
  return <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${m[code] ?? "bg-gray-100 text-gray-500"}`}>{l[code] ?? code}</span>;
}
...
export const DistributionTab: React.FC = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true); setError(null);
    try { setData(await getDistributions(getAuthSession()?.accessToken ?? "")); }
    catch (e) { setError(e instanceof Error ? e.message : "Lỗi tải phiếu phân phối"); }
    finally { setLoading(false); }
  }, []);

  useEffect(() => { void load(); }, [load]);

  const handleCreated = (d: Distribution) => { setShowCreate(false); setAckDist(d); void load(); };
